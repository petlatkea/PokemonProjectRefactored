...
public class TileDefinitions {
    ...
    private final Set<Integer> collisionTiles = Set.of(
    ...
    private final Set<Integer> grassTiles = Set.of(
            2, 3
    );
    ...
    public TileDefinitions() {
        tileTypes = new TileType[338];

        for (int i = 0; i <= 337; i++) {
            boolean hasCollision = collisionTiles.contains(i);
            boolean isGrass = grassTiles.contains(i);

            tileTypes[i] = new TileType(i, hasCollision, isGrass);
        }
    }
